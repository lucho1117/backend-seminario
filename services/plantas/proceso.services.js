...
const proceso = procesoModel(sequelize, Sequelize);
...
exports.findAll = async () => {
  return proceso.findAll({
      where: {
        activo: true,
      },
      order: [["idProceso", "DESC"]],
    })
    .then(response=>{
...
exports.deleteById = async (obj) => {
	...
	return proceso
		.update(aux, { where: { idProceso: obj.idProceso } })
		.then((response) => {
			return responsesServices.success(response);
		})
		.catch((error) => {
